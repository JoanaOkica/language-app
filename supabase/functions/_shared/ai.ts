...
export async function analyseSpeech(params: {
  ...
  const system =
    `You are FRED, an encouraging ${params.targetLanguage} speaking coach for a CEFR ` +
    `${params.level} learner. ${INJECTION_NOTICE} Reply with STRICT JSON only, using keys: ` +
...
export async function generateVocabulary(params: {
  ...
  const system =
    `You generate practical vocabulary for a CEFR ${params.level} learner of ` +
  ...
  const user = `Activity: <<<${clip(params.title, 120)}>>>`;
