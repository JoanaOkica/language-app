-- =============================================================================
-- 0001_schema.sql — Linguafox core schema
--
-- CO-TENANCY CONTRACT
-- This project shares a Supabase instance with other apps. Therefore:
--   * every object lives in the `linguafox` schema — nothing is created in,
...
--     are provisioned lazily by `linguafox.ensure_profile()` instead;
--   * storage buckets and their policies are prefixed `linguafox-`/`linguafox_`.
--
...
create schema if not exists linguafox;

comment on schema linguafox is
  'Linguafox language-learning app. Self-contained; safe to drop independently.';
...
grant usage on schema linguafox to authenticated, service_role;

-- ---------------------------------------------------------------------------
-- Enumerated types (constrain values in the database, not just the app)
-- ---------------------------------------------------------------------------
create type linguafox.proficiency_level as enum
  ('beginner', 'elementary', 'intermediate', 'advanced');
create type linguafox.task_status as enum
  ('requested', 'generating', 'generated', 'error');
create type linguafox.connection_status as enum
  ('pending', 'accepted', 'declined', 'blocked');
create type linguafox.challenge_status as enum
  ('pending', 'active', 'completed', 'declined', 'expired');

-- ---------------------------------------------------------------------------
-- profiles — 1:1 with auth.users. Client-editable fields only.
-- ---------------------------------------------------------------------------
create table linguafox.profiles (
  ...
  level            linguafox.proficiency_level not null default 'beginner',
...
create unique index profiles_username_uniq on linguafox.profiles (lower(username));
...
-- ---------------------------------------------------------------------------
create table linguafox.user_stats (
...
-- ---------------------------------------------------------------------------
create table linguafox.tasks (
  id          uuid primary key default gen_random_uuid(),
  user_id     uuid not null references auth.users (id) on delete cascade,
  title       text not null check (char_length(title) between 1 and 120),
  description text check (description is null or char_length(description) <= 500),
  status      linguafox.task_status not null default 'requested',
  level       linguafox.proficiency_level not null default 'beginner',
  created_at  timestamptz not null default now(),
  updated_at  timestamptz not null default now()
);
create index tasks_user_created_idx on linguafox.tasks (user_id, created_at desc);

-- ---------------------------------------------------------------------------
-- vocabulary — ONE CARD PER WORD (features A & B)
-- The unique index is what makes a repeated routine impossible to duplicate.
-- ---------------------------------------------------------------------------
create table linguafox.vocabulary (
  id               uuid primary key default gen_random_uuid(),
  user_id          uuid not null references auth.users (id) on delete cascade,
  task_id          uuid references linguafox.tasks (id) on delete set null,
...
create unique index vocabulary_user_word_uniq on linguafox.vocabulary (user_id, lower(word));
create index vocabulary_user_created_idx on linguafox.vocabulary (user_id, created_at desc);
...
-- ---------------------------------------------------------------------------
create table linguafox.vocabulary_examples (
  id                   uuid primary key default gen_random_uuid(),
  vocabulary_id        uuid not null references linguafox.vocabulary (id) on delete cascade,
  user_id              uuid not null references auth.users (id) on delete cascade,
  task_id              uuid references linguafox.tasks (id) on delete set null,
...
create unique index vocabulary_examples_context_uniq
  on linguafox.vocabulary_examples (vocabulary_id, lower(context));
create index vocabulary_examples_word_idx on linguafox.vocabulary_examples (vocabulary_id, created_at);
create index vocabulary_examples_user_idx on linguafox.vocabulary_examples (user_id, created_at desc);
...
create index challenges_challenger_idx on linguafox.challenges (challenger_id, status);
create index challenges_opponent_idx   on linguafox.challenges (opponent_id, status);
...
-- ---------------------------------------------------------------------------
create table linguafox.fred_sessions (
  id                 uuid primary key default gen_random_uuid(),
  user_id            uuid not null references auth.users (id) on delete cascade,
  challenge_id       uuid references linguafox.challenges (id) on delete set null,
  prompt             text,
...
create index fred_sessions_user_created_idx on linguafox.fred_sessions (user_id, created_at desc);
...
create table linguafox.usage_daily (
  user_id       uuid not null references auth.users (id) on delete cascade,
...
-- ---------------------------------------------------------------------------
create table linguafox.connections (
  id           uuid primary key default gen_random_uuid(),
  requester_id uuid not null references auth.users (id) on delete cascade,
  recipient_id uuid not null references auth.users (id) on delete cascade,
  status       linguafox.connection_status not null default 'pending',
  created_at   timestamptz not null default now(),
  updated_at   timestamptz not null default now(),
...
create unique index connections_unique_pair_idx on linguafox.connections (
  least(requester_id, recipient_id),
  greatest(requester_id, recipient_id)
);
create index connections_requester_idx on linguafox.connections (requester_id, status);
create index connections_recipient_idx on linguafox.connections (recipient_id, status);

-- ---------------------------------------------------------------------------
-- updated_at maintenance
-- ---------------------------------------------------------------------------
create or replace function linguafox.touch_updated_at()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
create trigger profiles_touch    before update on linguafox.profiles    for each row execute function linguafox.touch_updated_at();
create trigger tasks_touch       before update on linguafox.tasks       for each row execute function linguafox.touch_updated_at();
create trigger connections_touch before update on linguafox.connections for each row execute function linguafox.touch_updated_at();
create trigger challenges_touch  before update on linguafox.challenges  for each row execute function linguafox.touch_updated_at();
...
-- by the grant below.
-- ---------------------------------------------------------------------------
create view linguafox.public_profiles
with (security_invoker = off) as
  select
    ...
    s.star_points,
    s.streak_current
  from linguafox.profiles p
  join linguafox.user_stats s on s.user_id = p.id
  where p.is_public;

revoke all on linguafox.public_profiles from public;
grant select on linguafox.public_profiles to authenticated;
