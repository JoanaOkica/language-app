...
| 26 | **Cross-app interference** in a shared Supabase project | Everything lives in the `linguafox` schema; nothing is created in or revoked from `public`, no extensions are installed, and there is no trigger on the shared `auth.users`. | `0001`–`0005` |
