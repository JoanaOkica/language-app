...
An adversarial review of this model — including four findings that were fixed —
is in [PENTEST.md](PENTEST.md). Account-lifecycle specifics (password policy,
email confirmation, the 24-hour purge, reset and deletion) are in
[AUTH.md](AUTH.md).
...
| 25 | **Vocabulary bloat / duplicate-write abuse** | Unique indexes on `(user_id, lower(word))` and `(vocabulary_id, lower(context))` make repeats no-ops, so a loop of identical requests cannot inflate the table. | `0001` |
| 26 | **Cross-app interference** in a shared Supabase project | Everything lives in the `cats_tongue` schema; nothing is created in or revoked from `public`, no extensions are installed, and there is no trigger on the shared `auth.users`. | `0001`–`0005` |
| 27 | **Weak or breached passwords** | 12-char minimum, common-password blocklist, sequence/repeat detection, character-class mix and an email-similarity check client-side; Supabase Auth's length/complexity rules and HaveIBeenPwned check server-side. | `password.ts`, [AUTH.md](AUTH.md) |
| 28 | **Unverified accounts lingering** | No session until confirmation; `ensure_profile()` refuses unverified users at the database layer; unconfirmed signups are purged after 24 hours, scoped to this app only. | `0003`, `0005` |
| 29 | **Challenge integrity bypass** via unfrozen `expires_at` | Freeze trigger extended to `expires_at` and `kind` — see [PENTEST.md F-01](PENTEST.md). | `0002` |
| 30 | **Rate-limit laundering** across features | Per-action quota counters keyed by `kind`, so games cannot consume the AI budget — see [PENTEST.md F-02](PENTEST.md). | `0001`, `0003` |
| 31 | **Irreversible action with a stolen session** | Account deletion requires the current password, verified server-side — see [PENTEST.md F-03](PENTEST.md). | `delete-account` |
